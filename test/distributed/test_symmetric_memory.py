...
class SymmetricMemoryTest(MultiProcContinuousTest):
    ...
    def _run_lc_ag_variant_cuda_graph(self, op_name: str) -> None:
        ...
        Exercises the ping-pong signal-pad scheme under graph replay. A
        captured-constant counter (v2's pattern) would fail the second
        replay because the wait would observe the stale value from the
        prior replay; the ping-pong scheme, which restores IDLE at the end
        of each AG, should pass.
        """
        self._init_process()
    ...
    # NOTE: v3 and v5 allocate their (multicast-bound) output via
    # empty_strided_p2p() on every invocation, which performs a lazy
    # rendezvous on first use. rendezvous is a blocking collective and
    # therefore cannot be captured into a CUDA graph in eager mode.
    # Under inductor lowering this is a non-issue because CommBufferLayout
    # pre-allocates the output outside the captured region. A dedicated
    # eager-mode graph test for v3/v5 is left as a follow-up once a cached
    # workspace allocator is plumbed through the ops; the v4 graph test
    # covers the ping-pong signal-pad scheme that v3/v5 also rely on.
