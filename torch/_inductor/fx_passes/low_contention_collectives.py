...
def _replace_collective(
    ...
    if is_ag:
        # Selection order: v5 > v3 > v4 > v2 > v1. v3 and v5 require
        # multicast support; if absent we fall through to v4/v2/v1 as
        ...
        if target is None:
            if use_ag_v3 and _has_multicast_support(device_index):
                target = symm_mem._low_contention_all_gather_v3.default
            elif use_ag_v3 and not _has_multicast_support(device_index):
                log.info(
                    "low_contention_all_gather_v3 requested but multicast is "
                    "not supported on device %d; falling through.",
                    device_index,
                )

        if target is None and use_ag_v4:
